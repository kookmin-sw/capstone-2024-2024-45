...
고립된 현대사회, 이웃의 개념이 사라진 현재 고독사 인구 수는 연간 3,000명 이상이며, 증가 추세에 있다.
이웃간의 나눔을 촉진하여 공동체의 개념을 되살리는 것 만이, 이 심각한 사회 문제의 해결방법이다.

시간은행에서는 매칭되어 도움을 주고받은 사용자들 간에 댓가인 "시간"을 교환하는 방식으로 나눔 공동체 형성을 돕는다.
이로써, "같이"의 "가치"를 이룰 수 있게 된다.
  
### 📌개선한 기능

**1. QR을 이용한 간편한 송금**
  - 계좌번호 없이, QR코드를 스캔하는 것만으로 간편한 송금이 가능.
  - "유효시간"체커 기능을 도입해서 제 3자의 qr 도용을 방지
  - "hmac"을 도입하여 데이터 무결성과 인증을 보장
