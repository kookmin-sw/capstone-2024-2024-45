...
class GetPassword(GetAccount):
    ...
    async def get_password(self, account_id: str):
...
class ChangePassword(GetAccount):
    ...
    async def change_password(self, account_id, password):
...
class ChangeNickname(GetAccount):
    ...
    async def change_nickname(self, account_id, nickname):
        account_object = self.get_account_object(account_id)
...
class ChangeBalance(GetAccount):
    def __init__(self, session):
        super().__init__(session)

    async def change_balance(self, account_id, balance):
        account_object = self.get_account_object(account_id)
